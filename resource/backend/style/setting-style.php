...
if (!class_exists('PopupAllyStyleSettings')) {
	class PopupAllyStyleSettings {
		...
		public static function show_style_settings() {
			...
			$code = '<h3>Style Settings</h3>';

			$style_detail_template = PopupAllySettingShared::load_file_template(dirname(__FILE__) . '/setting-style-popup-template.php');
			foreach ($style as $id => $setting) {
				$code .= self::generate_individual_style_code($id, $setting, $style_detail_template);
			}
